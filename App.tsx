    ...
    <View>
      ...
      <View style={styles.container}>
        ...
        <Button title='Calcular' onPress={() => {
          ...
          Subtrair({text, onChangeResult});
          Multiplicar({text, onChangeResult});
          Dividir({text, onChangeResult});
        }}/>
...
function Multiplicar({text, onChangeResult}) {
  const valores: string[] = text.split('*');
  const resultado: number = Number(valores[0]) * Number(valores[1]);
...
function Subtrair({text, onChangeResult}) {
  const valores: string[] = text.split('-');
  const resultado: number = Number(valores[0]) - Number(valores[1]);
...
function Dividir({text, onChangeResult}) {
  const valores: string[] = text.split('/');
  const resultado: number = Number(valores[0]) / Number(valores[1]);
